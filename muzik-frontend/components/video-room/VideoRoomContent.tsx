...
const coursesData = {
  "course-1": {
    ...
    description:
      "This module covers the essential concepts and techniques in music production. You'll learn about digital audio workstations, signal flow, recording techniques, and basic mixing principles. By the end of this module, you'll have the foundation needed to start producing your own music.",
    lessons: [
      {
        id: 1,
        ...
        content:
          "This lesson walks you through the essential setup of a Digital Audio Workstation for music production.",
        otp: "20160313versASE323oFfLos1jd6YiTTNXlCALSuphYKMOID1Yu5Vd3DBM60XVnI",
        playbackInfo:
          "eyJ2aWRlb0lkIjoiMmI4NGFkYzIwZTE5NGZkMzgwYzZjZDYyMmFkNGYxZTMifQ==",
      },
      {
        id: 2,
        ...
        content:
          "Understanding signal flow is crucial for troubleshooting and creating complex audio production setups.",
        otp: "20160313versASE323sDFfLos1jd6YiTTNXlCALSuphYKMOID1Yu5Vd3DBM60XVnI",
        playbackInfo:
          "eyJ2aWRlb0lkIjoiM2I4NGFkYzIwZTE5NGZkMzgwYzZjZDYyMmFkNGYxZTMifQ==",
      },
      {
        id: 3,
        ...
        content:
          "Learn proper microphone placement, room treatment considerations, and performance techniques for capturing the perfect vocal take.",
        otp: "20160313versASE323oFfLos1jd6FGTTNXlCALSuphYKMOID1Yu5Vd3DBM60XVnI",
        playbackInfo:
          "eyJ2aWRlb0lkIjoiNGI4NGFkYzIwZTE5NGZkMzgwYzZjZDYyMmFkNGYxZTMifQ==",
      },
      {
        id: 4,
        ...
        content:
          "This lesson covers MIDI basics, from note input to advanced controller automation for expressive performances.",
        otp: "20160313versASE323oFfLos1jd6YiTBNXlCALSuphYKMOID1Yu5Vd3DBM60XVnI",
        playbackInfo:
          "eyJ2aWRlb0lkIjoiNWI4NGFkYzIwZTE5NGZkMzgwYzZjZDYyMmFkNGYxZTMifQ==",
      },
      {
        id: 5,
        ...
        content:
          "Learn the foundations of mixing including balance, EQ, compression, and spatial effects to create professional sounding mixes.",
        otp: "20160313versASE323oFfLos1jd6YiTTNHLCALSuphYKMOID1Yu5Vd3DBM60XVnI",
        playbackInfo:
          "eyJ2aWRlb0lkIjoiNmI4NGFkYzIwZTE5NGZkMzgwYzZjZDYyMmFkNGYxZTMifQ==",
      },
    ],
  },
...
export function VideoRoomContent({ courseId: string }) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [currentLesson, setCurrentLesson] = useState<any>({}); // null instead of {}
  ...
  const fetchVideoPlayListById = async (courseId: string) => {
    try {
      ...
      setStoredata(fetchData?.data?.at(0));
      console.log("ssdjgfgdsyigfisd", fetchData?.data?.at(0));
      setCurrentLesson(fetchData?.data?.at(0));
    } catch (error) {
      console.log("dzhksiusfdgs", error);
    }
  };
  const videoArray = storedata?.playlist?.at(0)?.modules;
  ...
  useEffect(() => {
    router.push(
      `/video-room?courseId=${courseId}&lessonId=${currentLesson.videourl}`
    );
  }, [currentLesson?.videourl, courseId, router]);

  // Handle video selection
  const handleVideoSelect = (video: any, idx: any) => {
    console.log("sdajbbfkugisd", video, idx, videoArray);

    const videoURL = videoArray?.filter(
      (item: any) => item?.url === video?.video?.url
    );
  ...
  const handleNextLesson = () => {
    const currentIndex = courseData.lessons.findIndex(
      (lesson) => lesson.id === currentLesson.videourl
    );
    ...
    <div className="container mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <VideoPlayer otp={currentLesson} playbackInfo={currentLesson} />
        </div>
        ...
        <div className="lg:col-span-1">
          <UpcomingVideosList
            ...
            currentVideoId={currentLesson?.idx}
          />
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
        {currentLesson?.courseTitle && (
          <VideoTitle title={currentLesson.courseTitle} />
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <button
            ...
            // disabled={
            //   currentLesson.id ===
            //   courseData.lessons[courseData.lessons.length - 1].id
            // }
          >
